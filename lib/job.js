...
function getPublicHistory(callback) {
  jobsModel.find({ public: true }, (err, data) => {
    const filtered = _.filter(data, (x) => { return _.get(x, 'data.title', false); });
    const sorted = _.reverse(_.sortBy(filtered, 'timestamp'));
    const limited = _.slice(sorted, 0, 10);
  ...
  get(id, (jobErr, job) => {
    ...
    jobsModel.upsert(job, (err1, data1) => {
      resolvers(job.url, job.type, (dataResolved) => {
        ...
        job.title = _.get(dataResolved, 'data.title');
        job.state = 'complete';
        job.lastResolvedTimestamp = new Date();
